...
export async function sendResetPasswordEmail(to: string, resetLink: string) {
    const mailOptions = {
        from: `"Motive SD" <${process.env.EMAIL_USER}>`,
        to,
        subject: "Motive SD - Reset Your Password",
        html: `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin:0;padding:0;background-color:#f4f4f4;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:520px;margin:40px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <tr>
      <td style="padding:40px;">
        <p style="margin:0 0 8px;color:#333;font-size:18px;font-weight:600;">Password Reset Request</p>
        <p style="margin:0 0 28px;color:#666;font-size:14px;line-height:1.6;">
        ...
        <!-- Reset Button -->
        <div style="text-align:center;margin:0 0 28px;">
          <a href="${resetLink}" style="display:inline-block;background:#2C2B29;color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;letter-spacing:1px;text-transform:uppercase;padding:16px 36px;border-radius:6px;">Reset Password</a>
        </div>
        <p style="margin:0 0 4px;color:#999;font-size:12px;line-height:1.5;">
