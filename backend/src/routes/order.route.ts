...
const calculateEstimatedDeliveryDate = (shippingMethod: string): Date => {
    const days = DELIVERY_DAYS[shippingMethod as keyof typeof DELIVERY_DAYS] || 7;
    const date = new Date();
...
const generatePaymentUrl = (orderCode: string, totalAmount: number): string => {
