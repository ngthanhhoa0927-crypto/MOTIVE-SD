...
cartRouter.get("/count", async (c) => {
    try {
        ...
        const result = await db.select({
            totalQuantity: sql `sum(${cartItems.quantity})::int`
        })
            .from(cartItems)
...
cartRouter.post("/", zValidator("json", addCartItemSchema, (result, c) => {
    if (!result.success) {
    ...
    try {
        ...
        const existingItems = await db.select()
            .from(cartItems)
            .where(and(eq(cartItems.cartId, cartId), eq(cartItems.productVariantId, product_variant_id)));
...
cartRouter.put("/:id", zValidator("json", updateCartItemSchema, (result, c) => {
    if (!result.success) {
        return c.json({ message: "Validation failed", errors: result.error.issues }, 400);
    }
}), async (c) => {
    try {
        ...
        const existingItems = await db.select().from(cartItems).where(and(eq(cartItems.id, cartItemId), eq(cartItems.cartId, cartId)));
        if (existingItems.length === 0) {
