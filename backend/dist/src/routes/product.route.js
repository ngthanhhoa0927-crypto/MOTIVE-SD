    ...
    try {
        ...
        const newProduct = await db.transaction(async (tx) => {
            ...
            if (existing.length > 0)
                throw new Error("DuplicateProductName");
...
productRouter.get("/", async (c) => {
    try {
        ...
        let allProducts;
        if (statusParam) {
            allProducts = await db.select().from(products).where(eq(products.status, statusParam));
        }
        else {
...
productRouter.put("/:id", authMiddleware, adminMiddleware, zValidator('json', createProductSchema, (result, c) => {
    if (!result.success) {
        return c.json({
            message: "Validation failed",
            errors: result.error.issues.map(i => ({ field: i.path.join('.'), message: i.message }))
