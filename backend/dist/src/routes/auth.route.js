...
// ----------------- API gửi OTP (đăng ký) -----------------
authRouter.post("/send-otp", zValidator('json', sendOtpSchema, (result, c) => {
    if (!result.success) {
        return c.json({
            message: "Validation failed",
            errors: result.error.issues.map(i => ({ field: i.path[0], message: i.message }))
...
// Change Password API
authRouter.put("/change-password", authMiddleware, zValidator('json', changePasswordSchema, (result, c) => {
    if (!result.success) {
        return c.json({
            message: "Validation failed",
...
authRouter.put("/me", authMiddleware, zValidator('json', updateProfileSchema, (result, c) => {
    if (!result.success) {
        return c.json({
            message: "Validation failed",
            errors: result.error.issues.map(i => ({ field: i.path[0], message: i.message }))
...
authRouter.put("/users/:id", authMiddleware, adminMiddleware, zValidator('json', adminUpdateUserSchema, (result, c) => {
    if (!result.success) {
    ...
    try {
        ...
        let dbUpdateData = { ...updateData, updatedAt: new Date() };
        if (status) {
            if (status === 'Active') {
...
authRouter.delete("/users/:id", authMiddleware, adminMiddleware, async (c) => {
    try {
        ...
        const adminId = adminPayload.sub;
...
authRouter.post("/request-reset", zValidator('json', requestResetSchema, (result, c) => {
    if (!result.success) {
        return c.json({ message: "Validation failed", errors: result.error.issues }, 400);
    }
}), async (c) => {
...
authRouter.post("/reset-password", zValidator('json', resetPasswordSchema, (result, c) => {
    if (!result.success) {
        return c.json({ message: "Validation failed", errors: result.error.issues }, 400);
    }
}), async (c) => {
...
authRouter.post("/forgot-password", zValidator('json', forgotPasswordSchema, (result, c) => {
    if (!result.success) {
        return c.json({
            message: "Validation failed",
            errors: result.error.issues.map(i => ({ field: i.path[0], message: i.message }))
...
authRouter.post("/verify-reset-otp", zValidator('json', verifyResetOtpSchema, (result, c) => {
    if (!result.success) {
        return c.json({
            message: "Validation failed",
            errors: result.error.issues.map(i => ({
    ...
    try {
        ...
        const validOtp = otpRecords.find(o => o.otp === otp && o.type === "reset_password" && !o.is_used);
        if (!validOtp) {
            return c.json({ message: "Invalid OTP" }, 400);
        }
...
authRouter.post("/users/:id/disable", authMiddleware, adminMiddleware, zValidator('json', disableUserSchema, (result, c) => {
    if (!result.success) {
        return c.json({
            message: "Validation failed",
            errors: result.error.issues.map(i => ({
...
authRouter.post("/users/:id/enable", authMiddleware, adminMiddleware, async (c) => {
    try {
        const userId = parseInt(c.req.param("id"));
        const adminPayload = c.get("jwtPayload");
        const adminId = adminPayload.sub;
...
authRouter.get("/users/:id/status", authMiddleware, adminMiddleware, async (c) => {
    try {
        const userId = parseInt(c.req.param("id"));
        const userRecord = await db
            .select({
...
authRouter.get("/audit-logs/user/:userId", authMiddleware, adminMiddleware, async (c) => {
    try {
...
authRouter.get("/audit-logs/action/:action", authMiddleware, adminMiddleware, async (c) => {
    try {
...
authRouter.get("/audit-logs", authMiddleware, adminMiddleware, async (c) => {
    try {
        const page = parseInt(c.req.query("page") || "1");
        const limit = parseInt(c.req.query("limit") || "20");
        const offset = (page - 1) * limit;
