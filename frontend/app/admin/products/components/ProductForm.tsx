...
export default function ProductForm({ initialData = null, isEditingMode = true, onSave, onCancel }: ProductFormProps) {
    ...
    const [weight, setWeight] = useState(initialData?.weight ? initialData.weight.toString().replace(/g/gi, '').trim() : '');
    ...
    // Variations (Strictly Size and Color as per Add Product layout)
    ...
    useEffect(() => {
        if (initialData) {
            ...
            if (initialData.variants && initialData.variants.length > 0) {
                setVariantsData(initialData.variants.map((v: any) => ({
                    ...
                    image_preview: v.signed_url || v.image_url || '',
                    is_active: v.is_active ?? true
                })));
                const sizes = [...new Set(initialData.variants.map((v: any) => v.size).filter(Boolean))] as string[];
                const colors = [...new Set(initialData.variants.map((v: any) => v.color).filter(Boolean))] as string[];
    ...
    const handleMainImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (const f of files.slice(0, remaining)) {
            if (f.size > 5 * 1024 * 1024) {
                alert(`File ${f.name} exceeds 5MB limit and will be skipped.`);
            } else {
    ...
    const handleVariantImgChange = async (idx: number, e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 5 * 1024 * 1024) {
            alert("Variant image exceeds 5MB limit");
            return;
        }
        try {
            const data = await handleFileUpload(file);
    ...
    const handleSubmit = async () => {
        ...
        if (status === 'Active') {
            ...
            const variantColors = new Set(variantsData.filter(v => v.is_active && v.color).map(v => v.color.trim()));
            const imageColors = new Set(images.map(img => img.color).filter(Boolean));
            for (const c of variantColors) {
                if (!imageColors.has(c)) {
                    newErrors.images = newErrors.images ? newErrors.images + `\nColor variant "${c}" requires at least one image` : `Color variant "${c}" requires at least one image`;
                }
            }
        }
        ...
        try {
            ...
            if (res.ok) {
                if (onSave) {
                    const success = await onSave(await res.json());
                    if (success) {
                        setErrors({});
                        return;
                    }
            ...
            console.error(error);
            setErrors({ ...newErrors, global: "Failed to save product. Check network connection." });
            window.scrollTo({ top: 0, behavior: 'smooth' });
        } finally {
    ...
    return (
        <div className="space-y-4 sm:space-y-6 pb-24 lg:pb-32 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="w-full lg:w-2/3 space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.05)] border border-gray-100/80 p-6">
                        ...
                        <div className="space-y-5">
                            ...
                            <div>
                                <label className="block text-[13px] font-bold text-gray-700 mb-2">Total Stock (Read Only)</label>
                                <div className="w-full px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-lg text-[13px] font-bold text-gray-600 select-none cursor-not-allowed">
                                    {variantsData.filter(v => v.is_active).reduce((acc, v) => acc + (parseInt(v.stock) || 0), 0)} items
                                </div>
                                <p className="text-[11px] font-medium text-gray-400 mt-1.5">Stock is managed automatically through product variations.</p>
                            </div>
                        </div>
                    </div>
                                ...
                                {isStatusOpen && (
                                    <div className="absolute z-40 top-full left-0 mt-1 w-full bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden py-1 animate-in fade-in zoom-in-95 duration-100">
                                        {['Draft', 'Active', 'Archived'].map(val => (
                                            ...
                                            >
                                                {val}
                                                {status === val && <svg className="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>}
                                            </button>
                                        ))}
                                    </div>
                                )}
                    ...
                    <div className="bg-white rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.05)] border border-gray-100/80 p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" value={material} onChange={e => setMaterial(e.target.value)} placeholder="e.g. 80% Wool, 20% Polyester" className="w-full px-4 py-2.5 bg-[#F9FAFB] border border-gray-200 rounded-lg text-[13px] font-medium placeholder-gray-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-[13px] font-bold text-gray-700 mb-2">Size Info</label>
                                <input type="text" value={sizeInfo} onChange={e => setSizeInfo(e.target.value)} placeholder="e.g. One Size Adjustable" className="w-full px-4 py-2.5 bg-[#F9FAFB] border border-gray-200 rounded-lg text-[13px] font-medium placeholder-gray-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-[13px] font-bold text-gray-700 mb-2">Care Instructions</label>
                                <input type="text" value={care} onChange={e => setCare(e.target.value)} placeholder="e.g. Dry clean only" className="w-full px-4 py-2.5 bg-[#F9FAFB] border border-gray-200 rounded-lg text-[13px] font-medium placeholder-gray-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-blue-500" />
                            </div>
                        </div>
                    </div>
