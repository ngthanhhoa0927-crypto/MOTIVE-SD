...
export default function ProductDetailsPage() {
    ...
    const displayVariants = displayData.displayVariants;
    const displayProduct = displayData.displayProduct;

    useEffect(() => {
        if (product) {
            const firstColor = displayData.displayProduct.colors[0]?.name;
            const firstSize = displayData.displayProduct.sizes[0];
            if (firstColor && displayProduct.colors.every((c: any) => c.name !== selectedColor)) setSelectedColor(firstColor);
            if (firstSize && !displayProduct.sizes.includes(selectedSize)) setSelectedSize(firstSize);
        }
    }, [product, displayData]);
    ...
    const currentVariant = displayVariants.find((v: any) => v.color === selectedColor && v.size === selectedSize);
    ...
    const displayImages = displayProduct.allImages || ["/images/placeholder-hat.png"];
    ...
    const currentPrice = currentVariant ? parseFloat(currentVariant.price).toFixed(2) : displayProduct.base_price;
                        ...
                        <div className="space-y-6 mb-10">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-sm font-bold text-gray-300 line-through mb-0.5">${displayProduct.original_price}</span>
                                    ...
                                    {displayProduct.colors.map((c) => (
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                const firstImgUrl = displayProduct.colorImages[c.name]?.[0];
                                                const idx = firstImgUrl ? displayProduct.allImages.indexOf(firstImgUrl) : 0;
                                ...
                                {displayProduct.shipping_class && (
                                    <div className="p-5 bg-emerald-50 rounded-3xl border border-emerald-100 group cursor-default transition-all hover:shadow-lg hover:shadow-emerald-900/5">
                                        <div className="flex items-center gap-4 mb-2">
                                            ...
                                            <span className="text-[11px] font-black text-emerald-800 uppercase tracking-widest">{displayProduct.shipping_class}</span>
                                        </div>
                                        <p className="text-[10px] font-bold text-emerald-600/70 ml-14">
                                            {displayProduct.lead_time ? `Delivery in ${displayProduct.lead_time} days` : 'Ships Next Business Day'}
                                        </p>
                                    </div>
                                )}
                            </div>
                                    ...
                                    <div className="bg-white rounded-[32px] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                                        ...
                                        <div className="p-8 pb-10 space-y-6">
                                            {displayProduct.material && (
                                                <div className="flex items-center justify-between border-b border-gray-50 pb-4"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Material</span><span className="text-[13px] font-bold text-gray-800 text-right max-w-[60%]">{displayProduct.material}</span></div>
                                            )}
                                            {displayProduct.size_info && (
                                                <div className="flex items-center justify-between border-b border-gray-50 pb-4"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Size Info</span><span className="text-[13px] font-bold text-gray-800 text-right max-w-[60%]">{displayProduct.size_info}</span></div>
                                            )}
                                            {displayProduct.care && (
                                                <div className="flex items-center justify-between border-b border-gray-50 pb-4"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Care Instruction</span><span className="text-[13px] font-bold text-gray-800 text-right max-w-[60%]">{displayProduct.care}</span></div>
                                            )}
                                            {displayProduct.weight && (
                                                <div className="flex items-center justify-between border-b border-gray-50 pb-4"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Weight</span><span className="text-[13px] font-bold text-gray-800 text-right max-w-[60%]">{Number(displayProduct.weight).toFixed(0)}g</span></div>
                                            )}
                                            {displayProduct.package_dimensions && (
                                                <div className="flex items-center justify-between"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Package Dimensions</span><span className="text-[13px] font-bold text-gray-800 text-right max-w-[60%]">{displayProduct.package_dimensions} cm</span></div>
                                            )}
                                            {!displayProduct.material && !displayProduct.size_info && !displayProduct.care && !displayProduct.weight && (
