...
function SearchContent() {
    ...
    // Derive flash sale dynamically from fetched products
    const flashSaleProducts = products.length > 0 ? products.slice(0, 6).map((p: any) => ({
        id: p.id,
        name: p.name,
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <section className="bg-[#1C3FAA] rounded-xl p-6 mb-8 relative group">
                            ...
                            <div ref={scrollRef} className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                {flashSaleProducts.length > 0 ? flashSaleProducts.map((item, idx) => (
                                    <Link href={`/user/productdetail/${item.id || idx}`} key={item.id || idx} className="block w-[140px] sm:w-[160px] md:w-[180px] flex-shrink-0 snap-start bg-white rounded-lg p-2.5 relative hover:shadow-lg transition cursor-pointer">
                                        ...
                                        <div className="flex items-baseline gap-1 mb-2">
                                            ...
                                            {item.oldPrice && <span className="text-gray-400 text-[9px] line-through">{item.oldPrice}</span>}
                                        </div>
                                    ...
                                    <div className="w-full text-center text-white/70 py-4 text-xs">Loading flash sale...</div>
                                )}
                            </div>
                        </section>
                                    ...
                                    {searchResults.map((item: any, i: number) => (
                                        <div key={item.id || i} className="border border-gray-100 rounded-lg p-3 hover:shadow-md transition relative group flex flex-col">
                                            <Link href={`/user/productdetail/${item.id || i}`} className="block aspect-square bg-gray-50 rounded mb-2 relative overflow-hidden flex items-center justify-center border border-gray-100">
                                                <Image
                                                    src={(item.images && item.images[0] && (item.images[0].signed_url || item.images[0].image_url)) || "/images/placeholder-hat.png"}
                                                    alt={item.name}
                                                    fill
